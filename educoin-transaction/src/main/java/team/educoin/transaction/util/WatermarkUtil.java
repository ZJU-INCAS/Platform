...
public class WatermarkUtil {
    ...
    static {
        try {
            pdfWaterMarkEmbedTool = ResourceUtils.getURL("classpath:static/watermark/pdf_watermark_embed.py").getPath();
            imageWaterMarkEmbedTool = ResourceUtils.getURL("classpath:static/watermark/image_watermark_embed.py").getPath();
    ...
    public static String embedWatermark(String filename, String owner, String buyer) throws IOException, InterruptedException {
        ...
        String waterMarkInfo = "owner:" + owner + " -- buyner:" + buyer;
        ...
        if (type.equals("pdf")){
            // 调用python脚本
            command = String.format("python %s %s %s %s %s", pdfWaterMarkEmbedTool, tmpFile, fileEmbed, waterMarkInfo, fileEmbedOut);
        } else if (imageContain){
            // 调用python脚本
            command = String.format("python %s %s %s %s", imageWaterMarkEmbedTool, fileEmbed, waterMarkInfo, fileEmbedOut);
        } else {
    ...
    public static String extractWatermark(String filename) throws InterruptedException, IOException {
        ...
        String toolUrl;
        if (imageContain){
            toolUrl = imageWaterMarkEmbedTool;
        } else if (type.equals("pdf")){
            toolUrl = pdfWaterMarkEmbedTool;
        } else {
